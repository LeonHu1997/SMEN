...
def train_one_epoch(model: torch.nn.Module,
    ...
    if args.mask_ratio == -1:
        args.mask_ratio = random.choice([6/16,8/16,10/16,12/16])
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        samples = F.interpolate(samples, size=(224, 224), mode='bilinear', align_corners=False)
        samples = min_max_normalize(samples) #.repeat(1, 3, 1, 1)
